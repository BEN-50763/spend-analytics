...
import logging
import re
from search_ddg_scrape_tesco_v1_1 import run_ddg_searcher
...
def query_tesco_api(search_item, referrer, count, session=None):
    ...
    for attempt in range(max_retries):
        try:
            ...
            response = session.post(url, headers=headers, impersonate="chrome", json=body)
...
def extract_best_match(target, candidates_dicts, model):
    ...
    # Else lets find the best match
        ...
        if result_dict["match_score"] < 95:
            ddg_result_dict = run_ddg_searcher(target, model, clean_string)
